...
const ServiceCard = ({ name, description, index, href }: { name: string; description: string; index: number; href: string }) => {
...
const ServicesSection = ({ services, titleText, subtitleText }: { services: any[]; titleText?: string; subtitleText?: string }) => {
    ...
    <section className="section bg-secondary">
      <div className="container">
        <div data-aos="fade-up">
          <h2 className="section-title">{titleText ?? t('home.services.title')}</h2>
          <p className="section-subtitle">{subtitleText ?? t('home.services.subtitle')}</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 xl:gap-10 mt-12 p-2">
          {items.map((service, index) => {
            const translation = service?.translations?.find((t:any) => t.language === locale) || service?.translations?.[0] || {};
            return (
              <ServiceCard key={index} name={translation?.name || ''} description={translation?.description || ''} index={index} href={withLocale('/services')} />
            );
          })}
        </div>
